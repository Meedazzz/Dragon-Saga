...
Сделан реальный прогон интерфейса в виртуальном Chromium через Playwright. Проверка открывает страницы как пользователь: на desktop, tablet и mobile, проверяет изображения, горизонтальные переполнения, карты Севера, PDF-документы Брина, Таро, активности, видео, локальный редактор текста, новый лорбук, расширенный бестиарий, военные машины Иллирии, Палантир, Анаптаниум и индивидуальные темы героев.
...
- `/Dragon-Saga/lor`
- `/Dragon-Saga/map/full-north`
- `/Dragon-Saga/map/north-humans`
- `/Dragon-Saga/map/north-elves`
- `/Dragon-Saga/map/north-dwarves`
- `/Dragon-Saga/brin/astaria`
- `/Dragon-Saga/brin/pursuing-peace`
...
- работает локальный редактор текста через `/admin`;
...
### 8. Локальный редактор текста

Добавлен и проверен режим редактирования текста прямо на страницах сайта. Он включается через `/admin` или `?admin=1`, сохраняет правки в localStorage, позволяет экспортировать `content-overrides.json` и затем положить его в `public/` для публикации правок на GitHub Pages.

### 9. Карты Севера и документы Брина

Добавлено и проверено:

- единый длинный атлас всего Севера;
- отдельная карта людского Севера;
- отдельная карта Севера эльфов;
- отдельная карта Севера дворфов;
- переключатель слоёв карты;
- PDF-страница Брина «Астария»;
- PDF-страница Брина «Мирный план с орками».
